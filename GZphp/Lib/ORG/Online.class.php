...
class Online extends Think {
   ...
   public function __construct(&$params='') {
		...
		if($_COOKIE['GZ_onlineid']){
			$this->sessionid = $_COOKIE['GZ_onlineid'];
		}else{
   ...
   public function write($sessID,$sessData) {
		$ip = get_client_ip();
		$username = $_COOKIE['GZ_username'] ? $_COOKIE['GZ_username'] : '';
		$groupid = $_COOKIE['GZ_groupid'] ? intval($_COOKIE['GZ_groupid']) : 4;
		$sessiondata = array(
							'sessionid'=>$sessID,
							'userid'=>intval($_COOKIE['GZ_userid']),
							'username'=>$username,
							'ip'=>$ip,
							'lastvisit'=>time(),
							'groupid'=> $groupid,
