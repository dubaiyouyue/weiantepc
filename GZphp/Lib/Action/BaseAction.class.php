 ...
 */
if(defined('APP_PATH')!='./GZphp' && !defined("GZPHP"))  exit("Access Denied");
...
class BaseAction extends Action
{
    ...
    public function _initialize() {
			...
			if(APP_LANG){
				...
				$T = F('config_'.$lang,'', './GZphp/Tpl/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
			...
			if($_COOKIE['GZ_auth']){
				...
				list($userid,$groupid, $password) = explode("-", authcode($_COOKIE['GZ_auth'], 'DECODE', $gzphp_auth_key));
				$this->_userid = $userid;
				$this->_username = $_COOKIE['GZ_username'];
				$this->_groupid = $groupid; 
				$this->_email = $_COOKIE['GZ_email'];
			}else{
				$this->_groupid = $_COOKIE['GZ_groupid']=4;
				$this->_userid =0;
			}
	...
	$pre = M($module)->where("id<$id")->order("id DESC")->find();
    $next = M($module)->where("id>$id")->order("id ASC")->find();
    $this->assign('pre',$pre);
